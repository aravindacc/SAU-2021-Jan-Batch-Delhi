...
// homeParentNoteList changed instead of useNoteListSubscribedToParentNoteListInTheHomeViewOfTheWebApp
export const homeParentNoteList = (
  filters,
  sort,
  disablePagination,
  skip: boolean = false
  ...
  const currentPage = useSubscription(GetNotesTabNoteListQuery, {
    noteFilters: filters,
  });
  ...
  const randomFunction = useMemo(() => {
    let innerlist = currentPage.data.concat(
    ...
    return innerlist;
  }, [disablePagination, startIndex, currentPage, isCurrentPageEmpty]);
