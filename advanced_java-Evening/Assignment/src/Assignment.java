...
public class Assignment {
	...
	private static Element copyLicenseNodevalid(Element source)
	{
	...
	private static Element copyLicenseNodeinvalid(Element source)
	{
	...
	private static void validInvalidFilter(NodeList firstNode)
	{
		...
		for(int i=0;i<firstNode.getLength();i++)
		{
			...
			if(diff > 0) 
			{
				valid_details.appendChild(copyLicenseNodevalid(currEl));
			}
			else
			{
				invalid_details.appendChild(copyLicenseNodeinvalid(currEl));
			}
		}
	...
	{		
		...
		for(int i=0;i<first.getLength();i++)
		{
			...
			validInvalidFilter(currEl.getElementsByTagName("License"));
		}
	...
	public static void main(String[] args) {
		try
		{
			...
			File inputFile2=new File("src/input2.xml");
			...
			match(nList,nList2,"NIPR_Number");
            ...
            StreamResult validstreamResult = new StreamResult(new File("src/validoutput.xml"));
            transformer.transform(validdomSource, validstreamResult);
             
            
            DOMSource invaliddomSource = new DOMSource(invalidDocument);
            StreamResult invalidstreamResult = new StreamResult(new File("src/invalidoutput.xml"));
